...
static inline char *font_key(const char *font_name, usize size);

static inline char *filename(const char *fullname);

static inline char *f_basename(const char *path);

static inline char *get_fullpath(const char *path, const char *name);
...
manager *init_manager(const char *title, int screen_width, int screen_height,
                      u32 sdl_flags, u32 win_flags, u32 ren_flags,
                      u32 img_flags, SDL_Color back_color) {
  ...
  mn->win =
      SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                       screen_width, screen_height, win_flags);
...
void build_menu_ui(manager *mn) {
  ...
  for (i = 0; i < 5; i++) {
    SDL_Texture *texture =
        create_text_texture(mn->ren, items[i], mn->font, mn->back_color);
    dict_insert(mn->textures, items[i], &texture);
  }
  int w = (mn->screen_width - 60) / 2, h = (mn->screen_height - 120) / 2;
  for (i = 0; i < 4; i++) {
    SDL_Rect rect = {20 + ((i % 2) * w) + ((i % 2) * 20),
                     10 + ((i / 2) * h) + ((i / 2) * 20), w, h};
    SDL_Texture *texture = DICT_GET_VAL(SDL_Texture *, mn->textures, items[i]);
    menu_buttons[i] =
        init_button(rect, mn->back_color, def_btn, texture, items[i], 100, 50);
  }
  // SDL_Texture *texture = DICT_GET_VAL(SDL_Texture *, mn->textures, "load");
  // SDL_Rect rect = {mn->screen_width/2-(DEF_BTN_SW/2), h+h+40, DEF_BTN_SW,
  // DEF_BTN_SH}; menu_buttons[4] = init_button(rect, mn->back_color, def_btn,
  // texture, "load", DEF_BTN_SW-40, DEF_BTN_SH-10);
}
...
void run_game(manager *mn) {
  ...
  if (mn->gm->over) {
    stop_clock(mn->clock);
    SDL_RenderCopy(mn->ren,
                   DICT_GET_VAL(SDL_Texture *, mn->textures, "YOU LOSE!"), NULL,
                   &state);
  }
  draw_grid(mn->gm);
  show_clock(mn->clock);
...
void build_game_ui(manager *mn) {
  ...
  SDL_Texture *losing =
      create_text_texture(mn->ren, "YOU LOSE!", mn->font, red);
...
void build_game(manager *mn, int row, int col, int tile_size, int mine_count) {
  mn->gm = init_game(mn, row, col, 0, 0, mn->clock, tile_size, mine_count);
}
